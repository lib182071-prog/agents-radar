...
export interface TopicRadarItem {
  ...
  video_hook: string;
  risk_level: "低" | "中" | "高";
  publish_priority: "今天必发" | "值得发" | "备用" | "不建议";
  xhs_body: string;
  xhs_tags: string[];
  comment_hook: string;
  video_script: string;
...
const PRIORITY_ORDER: Record<TopicRadarItem["publish_priority"], number> = {
  今天必发: 0,
  值得发: 1,
  备用: 2,
  不建议: 3,
...
function parseTopicRadarJson(raw: string): TopicRadarItem[] {
  ...
  return parsed.topics
    ...
    .map((t) => ({
      ...
      risk_level:
        t.risk_level === "高" || t.risk_level === "中" || t.risk_level === "低" ? t.risk_level : "低",
      publish_priority:
        t.publish_priority === "今天必发" ||
        t.publish_priority === "值得发" ||
        t.publish_priority === "备用" ||
    ...
    .sort((a, b) => {
      const p = PRIORITY_ORDER[a.publish_priority] - PRIORITY_ORDER[b.publish_priority];
      if (p !== 0) return p;
    ...
    const mdRows = topics
      .map(
        (t, i) =>
          ...
          `- 短视频钩子：${t.video_hook}\n` +
          `- 风险等级：${t.risk_level}\n` +
          `- 发布优先级：**${t.publish_priority}**\n` +
          `- 小红书正文：${t.xhs_body}\n` +
          `- 小红书标签：${t.xhs_tags.join(" ")}\n` +
          `- 评论区引导：${t.comment_hook}\n` +
          `- 短视频口播稿：${t.video_script}\n` +
          `- 发布前核实点：${t.verification_note}\n` +
          `- 评分理由：${t.reason}`,
      )
      .join("\n\n---\n\n");
